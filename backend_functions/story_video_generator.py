...
try:
    ...
    from .cloudflare_storage_manager import CloudflareStorageManager
except ImportError:
...
def generate_story_video(topic: str, script_length: str = "medium", voice: str = "alloy",
                        width: int = 576, height: int = 1024, fps: int = 24,
    ...
    try:
        ...
        # Stage 7: Upload to Cloudflare
        print(f"[STORY VIDEO] Stage 7: Uploading to Cloudflare...")
        stage_start = time.time()
        
        try:
            cloudflare_manager = CloudflareStorageManager()
            
            # Get the final video file path
            final_video_path = final_result.get("final_video_file")
            
            if final_video_path and os.path.exists(final_video_path):
                # Create a unique job ID for this video (if not provided)
                video_job_id = f"video_{int(time.time())}_{uuid.uuid4().hex[:8]}"
                ...
                if cloudflare_result["success"]:
                    # Add Cloudflare info to results
                    results["cloudflare_upload"] = {
                        "success": True,
                        "cloudflare_id": cloudflare_result["cloudflare_id"],
                        "cloudflare_url": cloudflare_result["cloudflare_url"],
                        "upload_duration": time.time() - stage_start,
                        "local_file_deleted": cloudflare_result.get("local_file_deleted", False)
                    }
                    
                    # Update final_video info with Cloudflare URL
                    results["final_video"]["cloudflare_url"] = cloudflare_result["cloudflare_url"]
                    results["final_video"]["cloudflare_id"] = cloudflare_result["cloudflare_id"]
                    
                    print(f"[STORY VIDEO] Cloudflare: Upload successful - {cloudflare_result['cloudflare_url']}")
                    
                else:
                    results["cloudflare_upload"] = {
                        "success": False,
                        "error": cloudflare_result.get("error", "Unknown upload error"),
                        "upload_duration": time.time() - stage_start
                    }
                    print(f"[STORY VIDEO] Cloudflare: Upload failed - {cloudflare_result.get('error')}")
            else:
                results["cloudflare_upload"] = {
                    "success": False,
                    "error": "Final video file not found for upload",
                    "file_path": final_video_path
                }
                print(f"[STORY VIDEO] Cloudflare: Video file not found - {final_video_path}")
            ...
            print(f"[STORY VIDEO] Cloudflare: Upload exception - {e}")
