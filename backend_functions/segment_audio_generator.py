...
def get_voice_for_character(character_data: Dict[str, Any], used_voices: List[str] = None) -> str:
    ...
    character_gender = character_data.get('gender', 'neutral').lower()
    
    # Get potential voices for this tone
    tone_voices = VOICE_TONE_MAPPING.get(voice_tone, ['rachel', 'adam'])
    
    # Filter voices by gender preference
    gender_filtered_voices = []
    for voice in tone_voices:
        voice_info = VOICE_CHARACTERISTICS.get(voice, {})
        voice_gender = voice_info.get('gender', 'neutral')
        
        # Match gender preference
        if character_gender in ['male', 'm'] and voice_gender == 'male':
            gender_filtered_voices.append(voice)
        elif character_gender in ['female', 'f'] and voice_gender == 'female':
            gender_filtered_voices.append(voice)
        elif character_gender in ['neutral', 'unknown', '']:
            gender_filtered_voices.append(voice)
    
    # Use gender-filtered voices if available, otherwise fall back to tone voices
    potential_voices = gender_filtered_voices if gender_filtered_voices else tone_voices
    
    # Try to find an unused voice first
    for voice in potential_voices:
        if voice not in used_voices:
            print(f"[VOICE MATCH] Gender: {character_gender}, Tone: {voice_tone} -> {voice}")
            return voice
    
    # If all preferred voices are used, still return the best match
    selected_voice = potential_voices[0] if potential_voices else 'rachel'
    print(f"[VOICE FALLBACK] Gender: {character_gender}, Tone: {voice_tone} -> {selected_voice} (all preferred used)")
