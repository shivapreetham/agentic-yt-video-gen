...
try:
    ...
    print("[CLOUDFLARE] boto3 not available, using simulation mode")
...
class CloudflareStorageManager:
    ...
    def _init_r2_client(self):
        ...
        try:
            ...
            account_id = os.environ.get('CLOUDFLARE_R2_ACCOUNT_ID') or self.cloudflare_config.get('account_id')
            access_key_id = os.environ.get('CLOUDFLARE_R2_ACCESS_KEY_ID') or self.cloudflare_config.get('access_key_id')
            secret_access_key = os.environ.get('CLOUDFLARE_R2_SECRET_ACCESS_KEY') or self.cloudflare_config.get('secret_access_key')
            
            if not all([account_id, access_key_id, secret_access_key]):
                print("[CLOUDFLARE] Warning: R2 credentials not found, falling back to simulation mode")
                return None
            ...
            # Test connection
            bucket_name = os.environ.get('CLOUDFLARE_R2_BUCKET_NAME', 'ai-video-generator')
            r2_client.head_bucket(Bucket=bucket_name)
            
            print(f"[CLOUDFLARE] Successfully connected to R2 bucket: {bucket_name}")
            return r2_client
            
        except Exception as e:
            print(f"[CLOUDFLARE] Failed to initialize R2 client: {e}")
            print("[CLOUDFLARE] Falling back to simulation mode")
            return None
    ...
    def upload_video_to_cloudflare(self, job_id: str, video_file_path: str, 
        ...
        try:
            ...
            cloudflare_result = self._upload_to_cloudflare(
            ...
            if cloudflare_result["success"]:
                ...
                upload_record = {
                    ...
                    "object_key": cloudflare_result.get("object_key", f"videos/{cloudflare_result['cloudflare_id']}"),
                    "filename": filename,
                    "file_size": file_size,
                    ...
                    "simulation": cloudflare_result.get("simulation", False)
                }
    ...
    def _upload_to_cloudflare(self, file_path: str, filename: str, file_size: int) -> Dict[str, Any]:
        """Upload video file to Cloudflare R2"""
        try:
            if not self.r2_client:
                # Fallback to simulation if R2 client not available
                return self._simulate_cloudflare_upload(file_path, filename, file_size)
            
            bucket_name = os.environ.get('CLOUDFLARE_R2_BUCKET_NAME', 'ai-video-generator')
            public_url = os.environ.get('CLOUDFLARE_R2_PUBLIC_URL', 'https://pub-03ff283e17364a388c9937161449a2c4.r2.dev')
            
            # Generate unique object key
            timestamp = int(time.time())
            cloudflare_id = f"video_{timestamp}_{filename.replace(' ', '_')}"
            object_key = f"videos/{cloudflare_id}"
            ...
            public_video_url = f"{public_url.rstrip('/')}/{object_key}"
            
            print(f"[CLOUDFLARE] Successfully uploaded {filename} to R2 in {upload_time:.2f}s")
    ...
    def _simulate_cloudflare_upload(self, file_path: str, filename: str, file_size: int) -> Dict[str, Any]:
        """Simulate Cloudflare upload (fallback when R2 not available)"""
        try:
            cloudflare_id = f"sim_{int(time.time())}_{filename.replace('.', '_')}"
            time.sleep(0.1)  # Simulate brief upload time
            
            return {
                "success": True,
                "cloudflare_id": cloudflare_id,
                "url": f"https://simulation.cloudflare.com/videos/{cloudflare_id}",
    ...
    def _delete_from_cloudflare(self, cloudflare_id: str) -> Dict[str, Any]:
        ...
        try:
            if not self.r2_client:
                # Fallback to simulation if R2 client not available
                return self._simulate_cloudflare_delete(cloudflare_id)
            
            bucket_name = os.environ.get('CLOUDFLARE_R2_BUCKET_NAME', 'ai-video-generator')
            
            # Find the record to get the object key
            record = None
            for job_id, job_record in self.storage_records.items():
                if job_record.get("cloudflare_id") == cloudflare_id:
                    record = job_record
                    break
            ...
            if not record:
                return {
                    "success": False,
                    "error": f"No record found for cloudflare_id: {cloudflare_id}"
                }
            
            object_key = record.get("object_key")
            if not object_key:
                # For backwards compatibility, construct object key from cloudflare_id
                object_key = f"videos/{cloudflare_id}"
            
            # Delete from R2
            self.r2_client.delete_object(Bucket=bucket_name, Key=object_key)
            
            print(f"[CLOUDFLARE] Successfully deleted {cloudflare_id} from R2")
            ...
            print(f"[CLOUDFLARE] Delete failed: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    
    def _simulate_cloudflare_delete(self, cloudflare_id: str) -> Dict[str, Any]:
        """Simulate Cloudflare delete (fallback when R2 not available)"""
        try:
