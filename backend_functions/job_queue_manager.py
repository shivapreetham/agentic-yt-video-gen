...
class VideoJob:
    ...
    voice: str = "alloy"
    width: int = 576  # Mobile vertical format
    height: int = 1024  # Mobile vertical format
    fps: int = 24
...
class JobQueueManager:
    ...
    def clear_queue(self) -> int:
        ...
        cleared_count = len(self._job_cache)
        self._job_cache.clear()
        self._job_video_mapping.clear()
        
        # Clear the persistent files
        try:
            if os.path.exists(self.queue_file):
                os.remove(self.queue_file)
            if os.path.exists(self.video_mapping_file):
                os.remove(self.video_mapping_file)
        except Exception as e:
            print(f"[JOB QUEUE] Warning: Could not clear persistent files: {e}")
        
        print(f"[JOB QUEUE] Cleared {cleared_count} jobs from queue")
