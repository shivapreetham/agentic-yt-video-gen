...
@app.route("/agentic/clear-queue", methods=["DELETE"])
def clear_job_queue():
    ...
    try:
        ...
        old_status = job_queue_manager.get_queue_status()
        
        # Clear the queue
        cleared_count = job_queue_manager.clear_queue()
        ...
        return jsonify({
            ...
            "new_status": job_queue_manager.get_queue_status(),
            "timestamp": datetime.now().isoformat()
        })
...
@app.route("/agentic/submit-high-level-topic", methods=["POST"])
def submit_high_level_topic():
    """Submit a high-level topic and generate specific video topics using Gemini"""
    try:
        if not topic_generation_agent or not job_queue_manager:
            return jsonify({"error": "Agentic components not initialized"}), 500
        ...
        high_level_topic = data.get("high_level_topic", "").strip()
        video_count = data.get("video_count", 3)
        
        if not high_level_topic:
            return jsonify({"error": "High-level topic is required"}), 400
        
        if video_count < 1 or video_count > 20:
            return jsonify({"error": "Video count must be between 1 and 20"}), 400
        
        print(f"[AGENTIC] Generating {video_count} video topics from high-level topic: {high_level_topic}")
        
        # Generate specific topics from high-level topic using Gemini
        generated_topics = topic_generation_agent.generate_topics_from_high_level(
            high_level_topic, video_count
        )
        
        if not generated_topics:
            return jsonify({
                "success": False,
                "error": "Failed to generate topics from high-level topic",
                "message": "Topic generation returned no results"
            }), 500
        
        # Add generated topics directly to job queue
        jobs_added = 0
        job_ids = []
        ...
        for topic in generated_topics:
            try:
                job_id = job_queue_manager.add_job(
                    ...
                    script_length="medium",
                    voice="alloy",
                    img_style_prompt=f"professional, {topic.domain}-themed, high quality"
                )
                jobs_added += 1
                job_ids.append(job_id)
                
            except Exception as e:
                print(f"[AGENTIC] Error adding job for topic '{topic.topic}': {e}")
        
        return jsonify({
            "success": True,
            "message": f"Successfully generated {len(generated_topics)} topics and added {jobs_added} jobs to queue",
            "high_level_topic": high_level_topic,
            "topics_generated": len(generated_topics),
        ...
        return jsonify({
            "success": False,
            "error": str(e),
            "message": "Failed to submit high-level topic"
        }), 500
